...
adata = sc.read('all_cells.h5ad')
adata = adata[adata.obs['cell_type'] == 'malignant cell',:]

original_adata = sc.read('preprocessed/spatial/GSM6592061_M15_symbol_corrected.h5ad')
original_adata.var_names = original_adata.var['gene_symbol']

# original_adata.var_names_make_unique()
original_adata = original_adata[adata.obs.index,:]


# print(adata.shape)

# adata = adata[adata.obs['cell_type'] == 'malignant cell',:]
original_adata.obs['predicted_response'] = pd.Categorical(adata.obs['predicted_response_prob'].map({0: 'Resistant', 1: 'Sensitive'}))
